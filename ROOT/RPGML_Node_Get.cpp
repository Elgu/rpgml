...
: Node( _gc, identifier, so, NUM_INPUTS, NUM_OUTPUTS, NUM_PARAMS )
{
  ...
  DEFINE_OUTPUT_INIT( OUTPUT_TYPE, "type", String , 0 );
  DEFINE_OUTPUT_INIT( OUTPUT_TYPE, "dims", int    , 0 );
  DEFINE_OUTPUT_INIT( OUTPUT_TYPE, "size", index_t, 1 );
}
...
bool Get::tick( void )
{
  ...
  GET_OUTPUT_IF_CONNECTED( OUTPUT_TYPE, type, String , 0 );
  ...
  if( type && (**type) != in_type )
  {
    (**type) = in_type;
    getOutput( OUTPUT_TYPE )->setChanged();
  }
