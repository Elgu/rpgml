...
if [ $# -lt 1 ]; then
  echo "Usage: new_Function_plugin.sh <PluginName>"
  exit 1
fi 

PluginName=$1
PluginHeader="RPGML_Function_${PluginName}.h"
PluginCPP="RPGML_Function_${PluginName}.cpp"
...
function filter() # in, out
{
  ...
  cat "$in" \
     | sed \
       -e "s/Function_TEMPLATE_h/Function_""$INCLUDE_GUARD_NAMESPACES""$PluginName""_h/g" \
...
filter "$(dirname $0)""/../templates/RPGML_Function_TEMPLATE.h" "$PluginHeader"
filter "$(dirname $0)""/../templates/RPGML_Function_TEMPLATE.cpp" "$PluginCPP"
